...
interface InitiativeTypeGaugeProps {
  data: DashboardData;
  /** Shared display mode: 'area' shows ha/ton, 'count' shows project counts */
  mode?: MetricMode;
}
...
export function InitiativeTypeGauge({ data, mode = 'area' }: InitiativeTypeGaugeProps) {
  ...
  // Derive the local ha/count mode from the shared section toggle. When the
  // pillar has no area/effect data (nature/co2) we always fall back to counts.
  const displayMode: 'ha' | 'count' = mode === 'area' && canHaMode ? 'ha' : 'count';
  ...
  return (
    <section className="w-full max-w-5xl mx-auto px-4 pt-4 pb-10 relative overflow-hidden">
      <div className="absolute right-8 bottom-8 opacity-[0.08] hidden lg:block pointer-events-none">
        <NatureWatermark animal="heron" size={110} className="scale-x-[-1]" />
      </div>
      <div className="absolute left-4 top-4 opacity-[0.07] hidden md:block pointer-events-none">
      ...
      <div className="flex items-center gap-2.5 mb-2">
        ...
        <InfoTooltip
          title="Hvem står bag projekterne?"
          content={
            <>
              <p>
