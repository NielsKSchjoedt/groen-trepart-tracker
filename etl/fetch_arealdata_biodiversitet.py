...
# Full DCE materialisation is intentionally large for Sprint 4 spatial overlays.
# The live 2026 layer is ~200 MB after slimming; keep a high ceiling so the
# monthly build still catches accidental runaway payloads without blocking the
# expected dataset.
MAX_DCE_MB = 250.0
...
def main(argv: list[str] | None = None) -> int:
    """
    Fetch Arealdata biodiversity layers used by Sprint 3/4.

    @param argv - Optional CLI arguments for tests
    @returns Process exit code
    @example main(["--full-dce"])
    """
    ...
    parser.add_argument(
        "--full-dce",
        action="store_true",
        help="Materialise the full 83k-feature DCE 30% layer for spatial overlays",
    )
