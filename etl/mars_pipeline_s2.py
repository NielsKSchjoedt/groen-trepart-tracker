...
def _relevant_pillars(n: float, e: float, a: float) -> tuple[str, ...]:
    pillars: list[str] = []
    if float(n or 0) > 0:
        pillars.append("nitrogen")
    if float(e or 0) > 0:
        pillars.append("extraction")
    if float(a or 0) > 0:
        pillars.append("afforestation")
    return tuple(pillars)


def _bump(triple: dict, pl: str, n: float, e: float, a: float) -> None:
    for pk in _relevant_pillars(n, e, a):
        b = triple[pk][pl]
        b["count"] += 1
...
def build_by_pipeline_phase_from_enriched_plans(plan_entries: list[dict]) -> dict:
    """
    Aggregate national pipeline from enriched dashboard plans — same logic as
    ProjectFunnel / computePillarProjects on the frontend.
    """
    triple = _pillar()
    canc = _empty_cancelled()
        ...
        for proj in plan.get("projectDetails", []):
            if proj.get("isCancelled") or proj.get("pipelinePhase") == "cancelled":
                ...
                if st in CANCELLED_STATES:
                    ...
                    _track_cancelled(canc, st, n, e, a)
                continue
            pl = proj.get("pipelinePhase", "preliminary_grant")
            n = proj.get("nitrogenT", 0) or 0
            e = proj.get("extractionHa", 0) or 0
            a = proj.get("afforestationHa", 0) or 0
            if pl == "sketch":
                sub = "ansoegt" if proj.get("statusNr") == 2 else "kladde"
                _bump_sketch(triple, n, e, a, sub)
            elif pl in MAIN_5:
                _bump(triple, pl, n, e, a)
    _round_five(triple)
    canc["totalCount"] = int(
